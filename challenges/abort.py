...
limiter = Limiter(app, key_func=get_remote_address, default_limits=["10 per day",
    "5 per hour"])

# if user sends GET to / (root)
@app.route("/")
def index():
    return render_template("abort.html")   # found in templates/
...
def login():
    ...
    if request.method == "POST":
        ...
        if request.form["username"] == "admin" :
            ...
            abort(412)  # if they didn't supply the username 'admin' send back a 401

...
def devicereboot(devicemd): # dictionary
    
    for ip in devicemd.keys(): # looping through the dictionary
        print(f'Connecting to...{ip}') #f string
        print("REBOOTING NOW")
    return None
...
def main():
    ...
    ## run
    commandpush(devicecmd) # call function to push commands to device
    devicereboot(devicecmd) # call function
